    ...
    <div className={`relative ${className}`}>
      ...
      <motion.div
        className={`absolute z-[30] rounded-2xl border border-white/20 bg-slate-900/40 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-xl ${
          compact
            ? '-left-4 top-[15%] w-24 sm:w-28'
            : '-left-4 top-[18%] w-28 sm:w-40 md:-left-24 lg:-left-32'
        }`}
      ...
      <motion.div
        className={`absolute z-[30] rounded-xl border border-white/10 bg-[#0c111d]/85 shadow-[0_25px_60px_rgba(0,0,0,0.6)] backdrop-blur-xl overflow-hidden ${
          compact
            ? '-right-4 bottom-[15%] w-32 sm:w-40'
            : '-right-4 bottom-[15%] w-36 sm:w-56 md:-right-28 lg:-right-36'
        }`}
        initial={{ opacity: 0, x: 30 }}
        animate={{ 
          opacity: 1, 
      ...
      <motion.div
        className={`absolute z-[30] rounded-2xl border border-red-500/30 bg-slate-950/60 p-3 sm:p-4 shadow-[0_20px_50px_rgba(220,38,38,0.2)] backdrop-blur-xl ${
          compact
            ? 'right-0 top-0 hidden'
            : 'right-0 top-[10%] w-28 sm:w-40 md:-right-20 lg:-right-28'
        }`}
      ...
      >
        ...
        <p className="text-[7px] sm:text-[9px] leading-tight text-slate-400 hidden sm:block">
      ...
      >
        <img
          src={profileSrc}
          alt={profileAlt}
          className="relative z-0 h-full w-full object-cover object-top brightness-[1.04] saturate-[1.06] contrast-[1.02]"
          width={880}
          height={1100}
